class Customer:
    def __init__(self, name: str, contact_information, delivery_address):
        self.name = name
        self.contact_information: ContactInformation = contact_information
        self.delivery_address: DeliveryAddress = delivery_address
...
class ContactInformation:
    ...
    def __validate_email(email_address: str) -> str:
        ...
        stripped_email = email_address.strip(' ')
...
class DeliveryAddress:
    ...
    def __validate_postcode(postcode: str) -> str:
        ...
        Raises: ValueError: If the postcode does not match the format LL09 0LL, where an L = Letter, 0 = mandatory number,
                             9 = optional number
        ...
        """
        # remove middle space
        stripped_postcode = postcode.strip(' ')

        if len(stripped_postcode) != 6 and len(stripped_postcode) != 7:  # originally in the format ZH1 0FR or ZH12 0GD
            raise ValueError("The postcode must be in the format LL09 0LL, where a L = Letter, 0 = mandatory number,"
                             "9 = optional number")
        # else
