class Bike:
    # Class Variables (i.e. not instance-related), declared using a tuple to make them immutable
    ...
    def get_components(self) -> list:
        """A getter function that returns the components used by this instance.

        Returns:
            list: A list of all components used by this instance.
             """
        #  A copy of the class variable 'components' is returned because the components used in child classes may differ
        #  to the list of available components specified in a class's variable. For instance, the SportBike class has
        #  the available component 'Drink Holder'. If this component is false, the component would originally still be
        #  used as the class is inheriting the method to check its own possible components rather than a separate list,
    ...
    def get_details(self) -> list[str]:
        """A getter function that returns the details of the class's properties. For example, the drink holder may be
        set to false which will be indicated here. In contrast, the 'get_components' function will not return this -
         it only returns the components that are used.

        Returns:
            list[string]: A list of string containing the details of each attribute in this instance.
        """
        # A dictionary is not used as the order is important.
        details = [
            f"Model: {self._model}", f"Size: {self.__size}", f"Colour: {self.__colour}",
            f"Handlebar Type: {self._handlebar_type}",  f"Wheel Size: {self.__wheel_size} inches",
            f"Gear Type: {self.__gear_type}", f"Brake Type: {self.__brake_type}", f"Light Option: {self.__light_option}"
        ]
    ...
    def __str__(self):
        return (f"\t Model: {self._model}, \n\t Size: {self.__size}, \n\t Colour: {self.__colour},"
                f"\n\t Handlebar Type: {self._handlebar_type}, \n\t Wheel Size: {self.__wheel_size} inches,"
...
class SportBike(Bike):
    ...
    def get_details(self) -> list[str]:
        ...
        details.append(f"Drink Holder: {self._drink_holder}")
        return details
...
class TourBike(Bike):
    ...
    # constructor. In contrast, the drink holder and mudflaps are optional.
    def __init__(
    ...
    def get_components(self) -> list[str]:
        ...
        # Rear Rack is also included as standard.
        additional_components = [component for component in additional_components if component] + ["Rear Rack"]

        return components_used + additional_components

    def get_details(self) -> list:
        details = super().get_details()
        new_components = [f"Drink Holder: {self._drink_holder}", f"Rear Rack: {self._rear_rack}",
                          f"Mudflaps: {self._mudflaps}"]
        return details + new_components
...
class CommuteBike(TourBike):
    ...
    def get_components(self) -> list:
        ...
        # Remove the rear rack as it does not come as standard
        components_used.remove("Rear Rack")  # the rear rack does not come as standard.
    ...
    def get_details(self) -> list[str]:
        details = super().get_details()
        new_components = ([f"Front Rack: {self.__front_rack}", f"Child Seat: {self.__child_seat}"])
        return details + new_components
